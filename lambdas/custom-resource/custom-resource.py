...
def process_rule_inclusion(priority, action, rule_type, protection_tag_name, rule_name, resource_properties, current_rules):
    update = None
    is_activated = True if (protection_tag_name is None or resource_properties[protection_tag_name] == "yes") else False
    ...
    if is_activated and rule_id not in current_rules:
        ...
        if 'GROUP' == rule_type:
            update['ActivatedRule']['OverrideAction'] = {'Type': action}
        else:
            update['ActivatedRule']['Action'] = {'Type': action}
...
def process_rule_exclusion(protection_tag_name, rule_name, resource_properties, old_resource_properties, current_rules):
    """
    SM: Update to handle the situation where you add more rule_names over time (like buying new marketplace rules.)
    """
    update = None
    rule_id = old_resource_properties[rule_name] if rule_name in old_resource_properties else None

    if rule_id is None:
        # Don't bother with the rest - this rule didn't exist in previous update of this custom resource.
        return update
    ...
    if was_activated and (not is_activated) and rule_id in current_rules:
        ...
        if 'GROUP' == rule_data['Type']:
            update['ActivatedRule']['OverrideAction'] = {'Type': rule_data['Action']}
        else:
            update['ActivatedRule']['Action'] = {'Type': rule_data['Action']}
...
def configure_web_acl(resource_properties, old_resource_properties):
    ...
    # Common rules
    updates.append(process_rule_inclusion(10, resource_properties['ActionWAFBlacklistRule'], 'REGULAR', None, 'WAFBlacklistRule', resource_properties, current_rules))
    updates.append(process_rule_inclusion(20, resource_properties['ActionWAFWhitelistRule'], 'REGULAR', None, 'WAFWhitelistRule', resource_properties, current_rules))
    ...
    updates.append(process_rule_inclusion(21, resource_properties['ActionF5BotDetectionSignaturesRuleGroup'], 'GROUP', 'ProtectionActivatedF5BotDetectionSignatures', 'F5BotDetectionSignaturesRuleGroup', resource_properties, current_rules))
    updates.append(process_rule_inclusion(22, resource_properties['ActionF5WebExploitsRulesRuleGroup'], 'GROUP', 'ProtectionActivatedF5WebExploitsRules', 'F5WebExploitsRulesRuleGroup', resource_properties, current_rules))
    ...
    if old_resource_properties:
        ...
        # Marketplace rule groups
        updates.append(process_rule_exclusion('ProtectionActivatedF5BotDetectionSignatures', 'F5BotDetectionSignaturesRuleGroup', resource_properties, old_resource_properties, current_rules))
        updates.append(process_rule_exclusion('ProtectionActivatedF5WebExploitsRules', 'F5WebExploitsRulesRuleGroup', resource_properties, old_resource_properties, current_rules))

    # ------------------------------------------------------------------------------------------------------------------
    # Clean invalid update elements
    # ------------------------------------------------------------------------------------------------------------------
    updates = [u for u in updates if u is not None]
    logging.getLogger().info("[configure_web_acl] cleaned updates: {}".format(updates))

    # ------------------------------------------------------------------------------------------------------------------
    # Clean IP sets before delete them
    # Skip marketplace rule groups by checking for "ActivatedRule.Action".
    # ------------------------------------------------------------------------------------------------------------------
    if old_resource_properties:
        rule_ids = [u['ActivatedRule']['RuleId'] for u in updates if (u['Action'] == 'DELETE' and 'Action' in u['ActivatedRule'])]
        ...
        if event['ResourceType'] == "Custom::ConfigureAppAccessLogBucket":
            ...
            if 'CREATE' in request_type:
                ...
                if (event['OldResourceProperties']['AppAccessLogBucket'] != event['ResourceProperties']['AppAccessLogBucket'] or
                    ...
                    remove_s3_bucket_lambda_event(
                        event['OldResourceProperties']["AppAccessLogBucket"],
            ...
            if 'CREATE' in request_type:
                ...
                if (event['OldResourceProperties']['WafLogBucket'] != event['ResourceProperties']['WafLogBucket'] or
                    ...
                    remove_s3_bucket_lambda_event(
                        event['OldResourceProperties']["WafLogBucket"],
                        old_lambda_app_log_parser_function
                    )
                    add_s3_bucket_lambda_event(
                        event['ResourceProperties']['WafLogBucket'],
            ...
            if 'CREATE' in request_type:
                put_logging_configuration(
                    event['ResourceProperties']['WAFWebACLArn'],
